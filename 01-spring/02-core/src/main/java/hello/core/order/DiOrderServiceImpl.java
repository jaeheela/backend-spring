...
//다양한 의존관계 주입 4가지 방법
//1. 생성자 주입 (lombok - @RequiredArgsConstructor)
...
// => 참고로 자바빈 프로퍼티, 자바에서는 과거부터 필드의 값을 직접 변경하지 않고, setXxx, getXxx 라는 메서드를 통해서 값을 읽거나 수정하는 규칙을 만들었는데,
// => 그것이 자바빈 프로퍼티 규약이다. 더 자세한 내용이 궁금하면 자바빈 프로퍼티로 검색해보자.
// => 자바빈 프로퍼티 규약 예시
//    class Data {
//      private int age;
...
// => 특징 : DI 프레임워크가 없으면 아무것도 할 수 없다.
// => 특징 : 사용하지 말자! 애플리케이션의 실제 코드와 관계 없는 테스트 코드, 스프링 설정을 목적으로 하는 @Configuration 같은 곳에서만 특별한 용도로 사용
// => 참고로 순수한 자바 테스트 코드에는 당연히 @Autowired가 동작하지 않는다. @SpringBootTest 처럼 스프링 컨테이너를 테스트에 통합한 경우에만 가능하다.
// => 참고로 다음 코드와 같이 @Bean 에서 파라미터에 의존관계는 자동 주입된다. 수동 등록시 자동 등록된 빈의 의존관계가 필요할 때 문제를 해결할 수 있다.
// =>  @Bean
//    OrderService orderService(MemberRepository memberRepoisitory, DiscountPolicydiscountPolicy) {
//          new OrderServiceImpl(memberRepository, discountPolicy)
//     }
//4. 일반 메서드 주입
// => 일반 메서드를 통해서 주입 받을 수 있다.
// => 특징 : 한번에 여러 필드를 주입 받을 수 있다. 일반적으로 잘 사용하지 않는다.
// => 참고로 어쩌면 당연한 이야기이지만 의존관계 자동 주입은 스프링 컨테이너가 관리하는 스프링 빈이어야 동작한다.
// => 스프링 빈이 아닌 Member 같은 클래스에서 @Autowired 코드를 적용해도 아무 기능도 동작하지 않는다.

//옵션 처리
// => 주입할 스프링 빈이 없어도 동작해야 할 때가 있다. - 예제로 확인해보자.
...
// => 출력결과 : setNoBean2 = null , setNoBean3 = Optional.empty
...
//2 누락
// => 프레임워크 없이 순수한 자바 코드를 단위 테스트 하는 경우에 수정자 의존관계인 경우 @Autowired가 프레임워크 안에서 동작할 때는 의존관계가 없으면 오류가 발생하지만,
// => 지금은 프레임워크 없이 순수한 자바 코드로만 단위 테스트를 수행하고 있다. 이렇게 테스트를 수행하면 실행은 된다.
...
// => 다음 코드를 보자. 잘 보면 필수 필드인 discountPolicy 에 값을 설정해야 하는데, 이 부분이 누락되었다. 자바는 컴파일 시점에 다음 오류를 발생시킨다.
// => private final MemberRepository memberRepository; private final DiscountPolicy discountPolicy;
// => @Autowired public OrderServiceImpl(MemberRepository memberRepository, DiscountPolicy discountPolicy) {this.memberRepository = memberRepository;}
...
// => 생성자 주입과 수정자 주입을 동시에 사용할 수 있다. 항상 생성자 주입을 선택해라!
...
// => 필드 주입처럼 좀 편리하게 사용하는 방법은 없을까? 역시 개발자는 귀찮은 것은 못 참는다! 다음 기본 코드를 최적화해보자.
...
@Component
public class DiOrderServiceImpl implements OrderService {

    private final MemberRepository memberRepository;
    private final DiscountPolicy discountPolicy;

    //조회 빈이 2개 이상일 경우의 문제
    // => @Autowired는 타입(Type)으로 조회한다. 그래서 마치 다음 코드와 유사하게 동작한다. (실제로는 더 많은 기능을 제공한다.)
    ...
    */
    ...
    //해결법1. @Autowired 필드 명 매칭
    // => @Autowired 는 타입 매칭을 시도하고, 이때 여러 빈이 있으면 필드 이름, 파라미터 이름으로 빈 이름을 추가 매칭한다. - 필드 명을 빈 이름으로 변경
    ...
    */
    ...
    // => 그러면 mainDiscountPolicy라는 이름의 스프링 빈을 추가로 찾는다. 하지만 경험상 @Qualifier는
    ...
    */
    ...
    /*
    //생성자 자동 주입 - 애노테이션 직접 만들어 사용
    @Autowired
    public OrderServiceImpl(MemberRepository memberRepository, @MainDiscountPolicy DiscountPolicy discountPolicy) {
        this.memberRepository = memberRepository;
        this.discountPolicy = discountPolicy;
    }
    //수정자 자동 주입 - 애노테이션 직접 만들어 사용
    @Autowired
    ...
    */
    ...
    /*
    //1. 생성자 주입 - 생성자가 딱 1개만 있으면 @Autowired 를 생략할 수 있다.
    //@Autowired
    public DiOrderServiceImpl(MemberRepository memberRepository, DiscountPolicy discountPolicy) {
        this.memberRepository = memberRepository;
        this.discountPolicy = discountPolicy;
    }
    */
    ...
    */
    ...
    public void setNoBean1(Member member) {
        System.out.println("setNoBean1 = " + member); //출력안됨
    }
