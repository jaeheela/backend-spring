...
public class BasicController {
    ...
    // => HTML의 콘텐츠(content)에 데이터를 출력할 때 :  <span th:text="${data}">
    // => HTML 테그의 속성이 아니라 컨텐츠 안에서 직접 출력할 때 = [[${data}]]
    //Escape
    // => HTML 문서는 <,> 같은 특수문자를 기반으로 정의 된다.
    ...
    // => HTML 엔티티는 < &lt; , > &gt; , 기타 매우 많다. HTML 엔티티와 관련해서 더 자세한 내용은 HTML 엔티티로 검색해보자.
    //http://localhost:8080/basic/01text-basic
    @GetMapping("/01text-basic")
    public String textBasic(Model model) {
        //model.addAttribute("data", "Hello Spring!"); //Hello Spring!
        model.addAttribute("data", "<b>Hello Spring!</b>"); //Hello <b>Spring!</b>
        return "basic/01text-basic";
    }

    //02. Unescape
    // => 이스케이프 기능을 사용하지 않으려면 어떻게 해야할까? 타임리프는 다음 두 기능을 제공한다.
    // => th:text -> th:utext
    // => [[...]] -> [(...)]
    // => th:inline="none" : 타임리프는 [[...]] 를 해석하기 때문에, 화면에 [[...]] 글자를 보여줄 수 없다. 이 테그 안에서는 타임리프가 해석하지 말라는 옵션이다.
    // => 실행해보면 다음과 같이 정상 수행되는 것을 확인할 수 있다. - 웹 브라우저: Hello Spring! >> 소스보기: Hello <b>Spring!</b>
    ...
    // => 변수 표현식 : ${...}
    // => 지역 변수 선언 : th:with 를 사용하면 지역 변수를 선언해서 사용 가능. 지역 변수는 선언한 테그 안에서만 사용가능
    // 1.Object
    ...
    public String basicObjects(HttpSession session) { //스프링 부트 3.0 미만
    ...
    */
    ...
    // => 컴포넌트스캔해서 HelloBean 객체가 스프링 빈으로 등록됨
    @Component("helloBean")
    ...
    // => [타임리프 자바8 날짜 지원 라이브러리 : thymeleaf-extras-java8time] , [자바8 날짜용 유틸리티 객체 - #temporals]
    ...
    // => [단순한 URL] : @{/hello} => /hello => http://localhost:8080/hello
    // => [쿼리 파라미터] : @{/hello(param1=${param1}, param2=${param2})} => /hello?param1=data1&param2=data2 : () 에 있는 부분은 쿼리 파라미터로 처리된다. : http://localhost:8080/hello?param1=data1&param2=data2
    // => [경로 변수] : @{/hello/{param1}/{param2}(param1=${param1}, param2=${param2})} -> /hello/data1/data2 : URL 경로상에 변수가 있으면 () 부분은 경로 변수로 처리된다. : http://localhost:8080/hello/data1/data2
    // => [경로 변수 + 쿼리 파라미터] : @{/hello/{param1}(param1=${param1}, param2=${param2})} -> /hello/data1?param2=data2 : 경로 변수와 쿼리 파라미터를 함께 사용할 수 있다. : http://localhost:8080/hello/data1?param2=data2
    // => 상대경로, 절대경로, 프로토콜 기준을 표현할 수도 있다. [/hello : 절대 경로] , [hello : 상대 경로] - 참고: https://www.thymeleaf.org/doc/tutorials/3.0/usingthymeleaf.html#link-urls
    ...
    // => int a = 10 * 20(10, 20는 숫자 리터럴)
    // => 참고) 이 내용이 쉬워 보이지만 처음 타임리프를 사용하면 많이 실수하니 잘 보아두자. 타임리프는 다음과 같은 리터럴이 있다.
    // => 문자: 'hello' - 타임리프에서 문자 리터럴은 항상 ' (작은 따옴표)로 감싸야 한다. -
    // => 숫자: 10
    // => 불린: true
    // => false null: null
    // 정상) <span th:text="'hello'"> : 그런데문자를항상 '로감싸는것은너무귀찮은일이다. 공백 없이 쭉 이어진다면 하나의 의미있는 토큰으로 인지해서 작은 따옴표를 생략할 수 있다.
    // => 하나의 토큰이 되는 룰: A-Z, a-z, 0-9, [], ., -, _
    // 오류) <span th:text="hello world!"/> : 문자 리터럴은 원칙상 ' 로 감싸야 한다. 중간에 공백이 있어서 하나의 의미있는 토큰으로도 인식되지 않는다.
    // 수정) <span th:text="'hello world!'"/> : 이렇게 ' 로 감싸면 정상 동작한다.
    ...
    // => 속성 설정 : th:* 속성을 지정하면 타임리프는 기존 속성을 th:* 로 지정한 속성으로 대체한다.기존 속성이 없다면 새로 만든다.
    // => 예시) <input type="text" name="mock" th:name="userA" /> -> 타임리프 렌더링 후 <input type="text" name="userA" />
    //속성 추가법
    // => th:attrappend : 속성 값의 뒤에 값을 추가한다.
    // => th:attrprepend : 속성 값의 앞에 값을 추가한다.
    // => th:classappend : class 속성에 자연스럽게 추가한다.
    // => checked 처리 : HTML에서는 <input type="checkbox" name="active" checked="false" /> 이 경우에도 checked 속성이 있기 때문에 checked 처리가 되어버린다.
    ...
    // => 예시) <input type="checkbox" name="active" th:checked="false" />
    ...
    // => 반복 기능 : <tr th:each="user : ${users}"> 반복시
    // => 오른쪽 컬렉션( ${users} )의 값을 하나씩 꺼내서 왼쪽 변수( user )에 담아서 태그를 반복 실행합니다.
    ...
    // => 반복 상태 유지 : <tr th:each="user, userStat : ${users}"> 반복의 두번째 파라미터를 설정해서 반복의 상태를 확인 할 수 있습니다.
    // => 두번째 파라미터는 생략 가능한데, 생략하면 지정한 변수명( user ) + Stat 가 됩니다.
    // => 여기서는 user + Stat = userStat 이므로 생략 가능합니다.
    //반복 상태 유지 기능
    // => [index : 0부터 시작하는 값] [count : 1부터 시작하는 값]
    // => [size : 전체 사이즈] [even , odd : 홀수, 짝수 여부(boolean)] [
    // => first , last :처음, 마지막 여부(boolean)] [current : 현재 객체]
    ...
    // 1. 텍스트 렌더링 : var username = [[${user.username}]];
    ...
    // 2. 다음으로 나오는 숫자 age의 경우에는 " 가 필요 없기 때문에 정상 렌더링 된다.
