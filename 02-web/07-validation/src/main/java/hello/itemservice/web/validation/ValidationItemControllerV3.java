...
// => 참고) 특정 필드의 범위를 넘어서는 검증(가격 * 수량의 합은 10,000원 이상) 기능이 빠졌는데, 이 부분은 조금 뒤에 설명한다.
...
// => 주의!) 다음과 같이 직접 글로벌 Validator를 직접 등록하면 스프링 부트는 Bean Validator를 글로벌 Validator 로 등록하지 않는다.
// => 따라서 애노테이션 기반의 빈 검증기가 동작하지 않는다. 다음 부분은 제거하자. - ItemServiceApplication.java
...
// => 둘중 아무거나 사용해도 동일하게 작동하지만, @Validated 는 내부에 groups 라는 기능을 포함하고 있다. 이 부분은 조금 뒤에 다시 설명하겠다.

//검증 순서
//=> @ModelAttribute 각각의 필드에 타입 변환 시도 >> 1.성공하면 다음으로 >> 2.실패하면 typeMismatch 로 FieldError 추가 >>  Validator 적용
