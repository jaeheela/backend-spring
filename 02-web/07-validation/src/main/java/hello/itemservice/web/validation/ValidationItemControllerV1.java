...
// => 2. 타입 오류 처리가 안된다. - Item 의 price,quantity 같은 숫자 필드는 타입이 Integer 이므로 문자 타입으로 설정하는 것이 불가능하다.
// => 3. 숫자 타입에 문자가 들어오면 오류가 발생한다. - 그런데 이러한 오류는 스프링MVC에서 컨트롤러에 진입하기도 전에 예외가 발생하기 때문에, 컨트롤러가 호출되지도 않고, 400 예외가 발생하면서 오류 페이지를 띄워준다.
...
public class ValidationItemControllerV1 {
    ...
    // => [GET] http://localhost:8080/validation/v1/items
    @GetMapping
    ...
    // => [GET] http://localhost:8080/validation/v1/items/1
    @GetMapping("/{itemId}")
    ...
    // => [GET] http://localhost:8080/validation/v1/items/add
    @GetMapping("/add")
    ...
    public String addItem(@ModelAttribute Item item, RedirectAttributes redirectAttributes, Model model) {
        ...
        if (!errors.isEmpty()) {
            log.info("errors = {} ", errors); //errors = {itemName=상품 이름은 필수입니다., quantity=수량은 최대 9,999 까지 허용합니다., price=가격은 1,000 ~ 1,000,000 까지 허용합니다.}
    ...
    // => [GET] http://localhost:8080/validation/v1/items/1/edit
    ...
    // => [POST] http://localhost:8080/validation/v1/items/1/edit
